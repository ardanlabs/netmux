...
func newClient() (local.LocalClient, error) {
	return local.NewClient(opts.User, opts.Pass)
}
...
func exit() error {
	...
	_, err = cli.Exit(context.Background(), &local.Noop{})
	return err
}
...
func epList() error {
	...
	res, err := ag.Config(context.Background(), &local.Noop{})
	if err != nil {
		return err
	}
...
func Run(log *logrus.Logger) error {
	...
	case "agent install":
		err := installer.Install(opts.Local.Install.Ctx, opts.Local.Install.Ns)
		if err != nil {
			return err
		}
	case "agent autoinstall":
		err := installer.AutoInstall(opts.Local.Autoinstall.Ctx, opts.Local.Autoinstall.Ns, opts.Local.Autoinstall.Arch)
		if err != nil {
			return err
		}
