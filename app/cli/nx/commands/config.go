...
func configSet() error {
	...
	_, err = cli.SetConfig(context.Background(), &local.StringMsg{Msg: fname})
	return err
}
...
func configGet() error {
	...
	res, err := cli.GetConfig(context.Background(), &local.Noop{})
	if err != nil {
		return err
	}
...
func configDump() error {
	...
	res, err := cli.Config(context.Background(), &local.Noop{})
	if err != nil {
		return err
	}
...
func hostsReset() error {
	...
	_, err = cli.ResetHosts(context.Background(), &local.Noop{})
	if err != nil {
		return err
	}
	return nil
