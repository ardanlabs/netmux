...
func Run(log *logrus.Logger, user *user.User) error {
	...
	cln, err := local.NewClient("", "")
	if err != nil {
		beeep.Alert("tray", fmt.Sprintf("ERROR: %s", err.Error()), "")
		return fmt.Errorf("local.NewClient: %w", err)
	}

	if _, err = cln.Load(context.Background(), &local.StringMsg{Msg: filepath.Join(user.HomeDir, ".netmux.yaml")}); err != nil {
		beeep.Alert("tray", fmt.Sprintf("ERROR: %s", err.Error()), "")
	...
	raFunc := func() {
		...
		if err != nil {
			log.Infof("tray: local.NewClient: %w", err)
			beeep.Alert("tray", fmt.Sprintf("ERROR: %s", err.Error()), "")
			return
		}

		client.Exit(context.Background(), &local.Noop{})
	}
	...
	var mu sync.RWMutex
	var client local.LocalClient

	go func() {
		defer wg.Done()

		for {
			log.Info("tray: waiting to connect to the local...")
			...
			mu.Lock()
			{
				log.Info("tray: connecting to the local...")
				client, err = local.NewClient("", "")
			}
			mu.Unlock()
			...
			if _, err := client.Exit(ctx, &local.Noop{}); err != nil {
	...
	mu.RLock()
	{
		_, err = client.Exit(ctx, &local.Noop{})
	}
	mu.RUnlock()
...
func processEvents(ctx context.Context, log *logrus.Logger, client local.LocalClient) error {
	for {
		if ctx.Err() != nil {
			return ctx.Err()
		}
		...
		events, err := client.Events(context.Background(), &local.Noop{})
		if err != nil {
